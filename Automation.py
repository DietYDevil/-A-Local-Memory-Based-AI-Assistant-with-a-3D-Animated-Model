...
from groq import Groq                          # Import AI chat functionalities.
...
classes = ["ZcUbw", "hGkELc", "LTKOO SYyric", "Z0LCW", "gsrt vk_bk FzvWSb YwPhnf", "pcIqee",
           "tw-Data-text tw-text-small", "IZ6rdc", "O5u6Rd LTKOO", "vlLzY6d", "webanswers-webanswers_table__webanswers",
           "d0No ibkb4b gsrt", "XSlAoe", "LwMfKe", "VQF4g", "q3vWpe", "kno-rdesc", "SPZz6b"]
...
SystemChatBot = [{"role": "system", "content": f"Hello, I am {os.environ['Username']}, You're a content writer. You have to write content like letters, codes, applications,essays, notes, songs,poems stc."}]
...
def Content(Topic):
    ...
    asyncio.run(TextToSpeech("the required content is on your screen, sir"))

    # Save the generated content to a text file.
    with open(f"Data/{Topic.replace(' ', '')}.txt", "w", encoding="utf-8") as file:
...
def YouTubeSearch(Topic):
    ...
    asyncio.run(TextToSpeech("search results are on your screen , sir"))
    return True  # Indicate success.

# Function to play a video on YouTube.
def PlayYoutube(query):
    playonyt(query)  # Use pywhatkit's playonyt function to play the video.
    asyncio.run(TextToSpeech("playing on youtube sir"))
    return True  # Indicate success.
...
def OpenApp(app, sess=requests.session):
    ...
    try:
        ...
        asyncio.run(TextToSpeech(f"opening {app}"))
        return True  # Indicate success.
...
#     asyncio.run(Automation(["play alone"]))
